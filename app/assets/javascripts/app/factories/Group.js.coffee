app.factory "Group1", ["$http", "$q", ($http, $q) ->
	groups = {}
	...
	groups.build_tree_menu = (id) ->
		...
		if groups.list.length > 1
			group = groups.find_group(id)
			group = groups.build_child(group)
			if group && group.ancestry != null
				array = group.ancestry.split("/").reverse()
				i = 0
				while i < array.length
					group = groups.build_parent(group, array[i])
					i++
			return group
